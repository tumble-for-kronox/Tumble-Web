...
export class FavouriteButtonComponent implements OnInit, OnDestroy {
  ...
  isBookmarkedSubscription!: Subscription;
  ...
  ngOnInit(): void {
    this.isBookmarkedSubscription = this.bookmarkService
      .isBookmark(this.programme.id)
      .subscribe((value) => {
  ...
  ngOnDestroy(): void {
    this.isBookmarkedSubscription.unsubscribe();
  }
