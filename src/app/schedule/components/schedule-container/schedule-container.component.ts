import { Component, ElementRef, HostListener, OnDestroy, Self, ViewChild } from '@angular/core';
...
export class ScheduleContainerComponent implements OnDestroy {
    ...
    currentColors$: Observable<Map<string, string>>;
    ...
    constructor(
        ...
        this.currentColors$ = this.colorService.currentColors;

        this._currentSelectedScheduleIdsSubscription = this.scheduleService.currentSelectedScheduleIds.subscribe(
            (scheduleIds) => {
        ...
        this.loadedSchedule.subscribe((schedule) => {
            ...
            this.assignColorsToScheduleEvents(schedule);
        });
    ...
    private assignColorsToScheduleEvents(schedule: Schedule) {
        schedule.days.forEach((day) => {
            day.events.forEach((event) => this.assignColorToEvent(event));
        });
    }

    private assignColorToEvent(event: Event) {
        const currentColors = this.colorService.currentColorsValue;
        const courseId = event.course.id;

        if (!currentColors.has(courseId)) {
            const randomColor = this.colorService.randomColor();
            this.colorService.updateSingleColor(courseId, randomColor);
        }
    }
}
