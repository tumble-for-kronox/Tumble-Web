...
export class SelectedSchedulesComponent {
  ...
  getSchedulesFromMultiSchoolSchedule(multiSchoolSchedules: MultiSchoolSchedules[] | null): string[] {
    if (!multiSchoolSchedules)
      return [];
  ...
  countSchedules(multiSchoolSchedules: MultiSchoolSchedules[] | null): number {
    ...
    return multiSchoolSchedules.flatMap(schedule => schedule.scheduleIds).length;
  }
